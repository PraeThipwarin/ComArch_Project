...
/**
 * แสดงสถานะเครื่องตามรูปแบบมาตรฐานของงาน (pc, memory, registers)
 * เรียกก่อน execute ทุกคำสั่ง และก่อนออก (ตอน halt)
 * @param statePtr pointer ไปยังสถานะเครื่อง
 */
...
/**
 * แปลงค่าล่าง 16 บิตให้เป็น signed 32 บิต (sign-extension)
 * @param num ค่า 16 บิต (two's complement)
 * @return ค่าแบบ signed 32 บิตหลัง sign-extend
 */
...
/**
 * ตรวจสอบว่า address อยู่ในช่วง [0..NUMMEMORY-1] มิฉะนั้นหยุดโปรแกรม
 * @param addr ที่อยู่หน่วยความจำ
 * @throw จบโปรแกรมด้วย exit(1) เมื่ออยู่นอกช่วง
 */
...
/**
 * จุดเริ่มโปรแกรม: โหลด machine code → ตั้งค่าเริ่มต้น → จำลอง fetch–decode–execute
 * จนเจอ halt โดยทำตามรูปแบบเอาต์พุตที่กำหนด
 * @param argc ต้องเป็น 2
 * @param argv argv[1] คือ path file machine code (เลขฐานสิบ บรรทัดละคำ)
 * @return 0 เมื่อสำเร็จ, 1 เมื่อใช้ผิดรูปแบบ/อ่านไฟล์ผิดพลาด/โปรแกรมใหญ่เกิน
 * @throw ไม่มีโดยตรง (แต่ assertMemInRange อาจ exit(1))
 */
